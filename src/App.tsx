...
// --- RESUMEN DE LA BIBLIOGRAFÍA PARA LA IA (CONTEXTO RAG) ---
const KNOWLEDGE_SUMMARY = `
BIBLIOGRAFÍA GENERAL (Docs 1-36):
- Leyes: Art. 3º (Inclusión/Excelencia), Ley General de Educación (NEM), LGDNNA (Interés Superior).
- Acuerdos: 30/09/24 (Alimentación), 14/12/23 (Acoso Escolar), 17/05/25 (Violencia Sexual - No revictimización), 05/04/24 (Consejos Técnicos Escolares).
- Procesos: PMC (Mejora Continua), Programa Analítico y Sintético, Autonomía Profesional.
- Autores: Antonio Bolívar (Familia y Capital Social), José Weinstein (Liderazgo Pedagógico), Molina Ruiz (Comunidades de Aprendizaje), Ezpeleta (Gestión que educa).

BIBLIOGRAFÍA ESPECIAL SUPERVISIÓN (Docs 37-40):
- Margarita Zorrilla: Transformación de la supervisión de vigilancia a asesoría técnica sistemática.
...
export default function App() {
  ...
  const fetchNewQuestion = async () => {
    ...
    const roleContext = activeLevel === 'Supervisión' 
      ? "Supervisor asesorando directores o gestionando zonas escolares." 
      : "Director resolviendo casos prácticos en un plantel.";

    const promptText = `Actúa como el Diseñador Jefe de exámenes de USICAMM México. 
    Tu tarea es generar un reactivo de opción múltiple (A, B, C) INÉDITO basado en un CASO PRÁCTICO.
    
    CONTEXTO PROFESIONAL:
    - Función: "${activeLevel}" (${roleContext})
    - Área: "${activeArea.title}"
    
    BASES TEÓRICAS (Obligatorio usar estos conceptos):
    ${KNOWLEDGE_SUMMARY}
    
    REQUISITOS DEL REACTIVO:
    1. Plantea un dilema técnico o ético realista (no memorístico).
    2. La respuesta correcta debe fundamentarse en la bibliografía citada arriba.
    3. Los distractores deben ser acciones administrativas comunes pero incorrectas bajo la NEM.
    
    RESPONDE EXCLUSIVAMENTE EN FORMATO JSON:
    {
      "type": "Cuestionamiento directo o Relación de elementos",
      "base": "Planteamiento detallado del caso práctico...",
      "options": [{"id": "A", "text": "..."}, {"id": "B", "text": "..."}, {"id": "C", "text": "..."}],
      "correct": "A",
      "argumentation": "Explicación técnica citando el documento oficial específico (ej. Según el Acuerdo 05/04/24... o Margarita Zorrilla menciona...)",
      "aiTip": "Un consejo estratégico corto para identificar la respuesta correcta."
    }`;
    ...
    while (retries > 0) {
      try {
        ...
        // Limpieza de formato markdown si existiera
        ...
        retries--;
        if (retries === 0) {
          setErrorMsg('El motor de IA está procesando mucha bibliografía. Intenta de nuevo.');
          setIsLoading(false);
        } else {
      ...
      <aside className={`fixed inset-y-0 left-0 w-80 bg-[#0f172a] text-slate-300 flex flex-col z-50 transition-transform duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-8 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-500 uppercase mt-1 tracking-widest text-shadow">Cerebro Gemini</p>
            </div>
          </div>
        ...
        <div className="p-6 bg-slate-900 border-t border-slate-800">
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-5 border border-slate-700 mb-4 text-center">
             <p className="text-[10px] font-black uppercase text-slate-500 mb-2 tracking-widest">Precisión IA</p>
        ...
        <div className="max-w-4xl mx-auto w-full px-4 py-8 lg:p-12">
          ...
          <div className="hidden lg:flex items-center justify-between mb-12">
            <div className="flex items-center gap-4">
              ...
              <div><h2 className="text-sm font-black text-slate-400 uppercase tracking-widest">Simulador IA Premium</h2><p className="text-xl font-bold text-slate-800">{activeLevel} • {activeArea.title}</p></div>
            </div>
            <div className="bg-emerald-500/10 text-emerald-600 px-4 py-2 rounded-full text-[10px] font-black flex items-center gap-2 border border-emerald-500/20">
               <FileCheck size={14} /> 40 DOCS CONECTADOS
            </div>
          </div>
          ...
          {!currentQuestion && !isLoading && !errorMsg && (
            <div className="bg-white rounded-[2.5rem] p-10 lg:p-20 text-center shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 duration-700">
              ...
              <h3 className="text-4xl lg:text-5xl font-black text-slate-900 mb-6 tracking-tight text-balance leading-none">Generación Infinita con Gemini Pro</h3>
              <p className="text-slate-500 text-lg lg:text-xl mb-12 max-w-xl mx-auto leading-relaxed">Cada reactivo es creado en tiempo real basándose estrictamente en la bibliografía oficial 2026. Dile adiós a las preguntas repetidas.</p>
              
              <button onClick={fetchNewQuestion} className="w-full lg:w-auto bg-[#0f172a] text-white px-12 py-5 rounded-[1.5rem] font-black text-xl hover:scale-105 transition-all shadow-xl flex items-center justify-center gap-4 mx-auto">
          ...
          {errorMsg && (
            <div className="p-12 bg-white rounded-[2.5rem] shadow-xl border border-red-100 text-center">
              ...
              <button onClick={fetchNewQuestion} className="bg-red-500 text-white px-10 py-4 rounded-2xl font-bold hover:bg-red-600 transition-all">Reintentar Ahora</button>
            </div>
          )}
              ...
              <div className="bg-white rounded-[2rem] shadow-2xl border border-slate-100 overflow-hidden">
                ...
                <div className="p-8 lg:p-12 space-y-4">
                  {currentQuestion.options.map((opt) => {
                    ...
                    if (!isEvaluated) {
                      style += isSelected ? "border-[#0f172a] bg-[#0f172a] text-white shadow-2xl scale-[1.02]" : "border-slate-100 bg-white hover:border-emerald-200 hover:bg-emerald-50/30";
                    } else {
